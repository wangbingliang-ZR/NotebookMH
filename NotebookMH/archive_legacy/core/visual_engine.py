...
class LandscapeRenderer:
    ...
    def render(
        ...
        if len(subgraph.nodes) >= 4:
            ...
            mz_vals = tuple(float(subgraph.nodes[n].get("mastery", 50.0)) for n in subgraph.nodes if n in safe_layout)
...
def assemble_dag_nodes(vault_uuid: str, user_id: str) -> List[Dict[str, Any]]:
    ...
    TODO: 后续建议迁移到独立的 query/service 层，避免 core/visual 反向依赖 DAO 细节。
    """
    logger.info("[MIGRATION_NOTICE] assemble_dag_nodes is temporarily located in visual_engine.py; consider moving to a dedicated service layer.")
    import json
