...
class CognitiveEngine:
    ...
    async def generate_response(
        ...
        chunks = await self._pipeline.retrieve(user_input, vault_uuid, top_k=top_k)
        context = self._build_context(chunks)

        if current_state == CognitiveState.LEARNING:
            return await self._learning_flow(user_input, context, strategy, profile, evaluation)
