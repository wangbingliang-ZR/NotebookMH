...
async def generate_quiz(vault_uuid: str, count: int = 20) -> list[dict]:
    ...
    data = await _gen_json(
        vault_uuid,
        system="你是资深中考命题专家，熟悉各地中考真题风格。请严格根据资料内容判断科目，生成对应科目的模拟卷。仅返回 JSON。",
        task=(
            f"基于资料生成一套完整的「{count}题中考模拟卷」，必须严格匹配真题难度和题型分布。\n\n"
            "请根据资料内容判断具体科目（如地理、生物、物理、化学、数学等），生成对应科目的题目。\n\n"
            "题型与题量（参照真实中考）：\n"
            "1. 单项选择题（约 40%）：每题 4 个选项\n"
            "2. 填空题（约 20%）：填关键概念、公式、术语等\n"
            "3. 读图/图表分析题（约 25%）：给示意图/图表/数据，设 2-3 小问\n"
            "4. 综合探究题（约 15%）：结合时事热点或生活情境，考查知识迁移\n\n"
            "命题要求：\n"
            "- 严格根据已收录资料中的真题风格和考点分布出题，不得编造资料中没有的内容\n"
            "- 每题标注【题型】和【分值】\n"
            "- 给出标准答案和详细解析（说明考查知识点）\n"
