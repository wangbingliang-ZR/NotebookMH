...
def render_holographic_console() -> None:
    ...
    with st.sidebar:
        ...
        if is_deadlocked:
            ...
            _render_sandbox_trigger()
            _render_biofeedback(grayed=True)
            _render_strategy_terminal(grayed=True)
            return

        # ── 正常遥测 ────────────────────────────────────────
        _render_biofeedback(grayed=False)
        _render_strategy_terminal(grayed=False)
        _render_cognitive_landscape_panel()
        _render_embodied_language_panel()
        _render_persistent_signal_badge()
        _render_thought_stream()

    # ── 主屏相变 toast（非侧边栏）─────────────────────────
    _render_phase_transition()
