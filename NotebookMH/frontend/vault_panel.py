...
def render() -> None:
    ...
    if st.sidebar.button("🔄 切换到选中库", key="btn_switch_vault"):
        if selected_uuid != current_vault:
            st.session_state["vault_uuid"] = selected_uuid
            binder.update_state("vault_uuid", selected_uuid)
            st.sidebar.success(f"已切换到：{vault_options.get(selected_uuid, selected_uuid)}")
            st.rerun()
        else:
            st.sidebar.info("已经是当前库")

    # ── 2. 新建 Vault ─────────────────────────────────
    with st.sidebar.expander("➕ 新建笔记库"):
    ...
    # ── 3. 删除 Vault ─────────────────────────────────
    with st.sidebar.expander("⚠️ 删除笔记库"):
